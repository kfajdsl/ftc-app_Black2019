    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (!foundGold) {
            ...
            if (SensorMethods.isGold()) {
                ...
                telemetry.update();
                DriveMethods.turn(); //turn into 2nd sample
                DriveMethods.driveDist(); //run into 2nd sample
                DriveMethods.turn(); //turn towards depot
                DriveMethods.driveDist(); //drive to depot
                DriveMethods.turn(); //turn to proper orientation
            }
            if (!foundGold) {
                ...
                if (SensorMethods.isGold()) {
                    ...
                    telemetry.update();
                    DriveMethods.turn(); //turn into 3rd sample
                    DriveMethods.driveDist(); //run into 3rd sample
                    DriveMethods.turn(); //turn towards depot
                    DriveMethods.driveDist(); //drive to depot
                    DriveMethods.turn(); //turn to proper orientation
                }
